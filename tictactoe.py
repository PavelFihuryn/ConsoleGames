...
Правила:
Для того, чтобы поставить ход используйте цифры:
    7  | 8 | 9
    ___|___|___
    4  | 5 | 6
    ___|___|___
...
def set_player_move(li):
    while True:
        ...
        if mo not in '1 2 3 4 5 6 7 8 9'.split():
            ...
            continue
        elif not is_space_free(li, int(mo) - 1):
...
def is_can_win(li: list, letter: str):
    for i in range(0, 9):
        copy = li.copy()
        copy[i] = letter
        if is_win(copy, letter) and is_space_free(li, i):
            return i
    ...
    for i in range(0, 9):
        copy = li.copy()
        copy[i] = letter_men
        if is_win(copy, letter_men) and is_space_free(li, i):
            return i
    for i in '0 2 6 8'.split():
        if li[int(i)] == ' ':
            return int(i)
    if li[4] == ' ':
        return 4
